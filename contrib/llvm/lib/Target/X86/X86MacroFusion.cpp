...
static X86::FirstMFInstKind classifyFirst(const MachineInstr &MI) {
  return X86::classifyFirstOpcode(MI.getOpcode());
}

static X86::SecondMFInstKind classifySecond(const MachineInstr &MI) {
  X86::CondCode CC = X86::getCondFromBranch(MI);
  return X86::classifySecondCondCode(CC);
}
...
static bool shouldScheduleAdjacent(const TargetInstrInfo &TII,
  ...
  const X86::SecondMFInstKind BranchKind = classifySecond(SecondMI);

  if (BranchKind == X86::SecondMFInstKind::Invalid)
    return false; // Second cannot be fused with anything.

  if (FirstMI == nullptr)
    return true; // We're only checking whether Second can be fused at all.

  const X86::FirstMFInstKind TestKind = classifyFirst(*FirstMI);

  if (ST.hasBranchFusion()) {
    // Branch fusion can merge CMP and TEST with all conditional jumps.
    return (TestKind == X86::FirstMFInstKind::Cmp ||
            TestKind == X86::FirstMFInstKind::Test);
  }
