...
-- Execute a command and return its output.  A final newlineis stripped,
-- similar to sh.
...
local function read_to(from_branch, to_branch, dirspec)
	...
	command = command .. " | sed -E -n 's/^[[:space:]]*\\(cherry picked from commit ([0-9a-f]+)\\)[[:space:]]*$/\\1/p'"
	if verbose > 1 then
...
local function usage(from_branch, to_branch, author)
	print("usage: $(basename $0) [-ah] [-f from_branch] [-t to_branch] [-u user] [-X exclude_file] [path ...]")
	print()
