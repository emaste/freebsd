...
static void db_print_stack_entry(
    const char *, int, char **, int *, db_addr_t, void *);
static void decode_syscall(int, struct thread *);

/*
 * Figure out how many arguments were passed into the frame at "fp".
 */
static int db_numargs(fp) struct i386_frame *fp;
{
	char *argp;
	int inst;
	int args;
...
static void db_print_stack_entry(name, narg, argnp, argp, callpc, frame) const
    char *name;
int narg;
char **argnp;
int *argp;
...
db_nextframe(struct i386_frame **fp, db_addr_t *ip, struct thread *td)
{
	...
	if (eip >= PMAP_TRM_MIN_ADDRESS) {
		sym = db_search_symbol(
		    eip - 1 - setidt_disp, DB_STGY_ANY, &offset);
	} else {
...
#define MAXNARG 16
	...
	while (count-- && !db_pager_quit) {
		...
		if (actframe != frame) {
			...
			pc = (db_addr_t)db_get_value(
			    (int)&actframe->f_retaddr, 4, false);
			continue;
		}
