...
struct scsi_attrib_table_entry {
	...
	int (*to_str)(struct sbuf *sb, struct scsi_mam_attribute_header *hdr,
	    uint32_t valid_len, uint32_t flags, uint32_t output_flags,
...
#define INQ_DATA_TQ_ENABLED(iqd)                               \
	((SID_ANSI_REV(iqd) < SCSI_REV_SPC2) ?                 \
		      ((iqd)->flags & SID_CmdQue) :                  \
		      (((iqd)->flags & SID_CmdQue) &&                \
		    !((iqd)->spc2_flags & SPC2_SID_BQueue)) || \
		    (!((iqd)->flags & SID_CmdQue) &&           \
			((iqd)->spc2_flags & SPC2_SID_BQueue)))
...
#define SBDC_IS_PRESENT(bdc, length, field)                                  \
	((length >=                                                          \
	     offsetof(struct scsi_vpd_block_device_characteristics, field) + \
		 sizeof(bdc->field)) ?                                       \
		      1 :                                                          \
		      0)
...
#define SSD_FIXED_IS_FILLED(sense, field)                           \
	((((offsetof(struct scsi_sense_data_fixed, field) +         \
		  ...
		  sizeof(sense->extra_len))) <= sense->extra_len) ? \
		      1 :                                                 \
		      0)
};
...
scsi_sense_action scsi_error_action(struct ccb_scsiio *csio,
    struct scsi_inquiry_data *inq_data, u_int32_t sense_flags);
...
int scsi_get_sks(
    struct scsi_sense_data *sense_data, u_int sense_len, uint8_t *sks);
...
struct scsi_vpd_id_descriptor *scsi_get_devid(
    struct scsi_vpd_device_id *id, uint32_t len, scsi_devid_checkfn_t ck_fn);
struct scsi_vpd_id_descriptor *scsi_get_devid_desc(
    struct scsi_vpd_id_descriptor *desc, uint32_t len,
    scsi_devid_checkfn_t ck_fn);

int scsi_transportid_sbuf(
    struct sbuf *sb, struct scsi_transportid_header *hdr, uint32_t valid_len);

const char *scsi_nv_to_str(
    struct scsi_nv *table, int num_table_entries, uint64_t value);
...
int scsi_attrib_volcoh_sbuf(struct sbuf *sb,
    struct scsi_mam_attribute_header *hdr, uint32_t valid_len, uint32_t flags,
...
struct scsi_attrib_table_entry *scsi_find_attrib_entry(
    struct scsi_attrib_table_entry *table, size_t num_table_entries,
    uint32_t id);
...
void scsi_attrib_prefix_sbuf(struct sbuf *sb, uint32_t output_flags,
    struct scsi_mam_attribute_header *hdr, uint32_t valid_len,
    const char *desc);
...
void scsi_mode_sense_subpage(struct ccb_scsiio *csio, uint32_t retries,
    void (*cbfcnp)(struct cam_periph *, union ccb *), uint8_t tag_action,
...
void scsi_read_capacity(struct ccb_scsiio *csio, u_int32_t retries,
    void (*cbfcnp)(struct cam_periph *, union ccb *), u_int8_t tag_action,
    struct scsi_read_capacity_data *, u_int8_t sense_len, u_int32_t timeout);
...
void scsi_synchronize_cache(struct ccb_scsiio *csio, u_int32_t retries,
    void (*cbfcnp)(struct cam_periph *, union ccb *), u_int8_t tag_action,
...
int scsi_devid_match(
    uint8_t *rhs, size_t rhs_len, uint8_t *lhs, size_t lhs_len);

void scsi_extract_sense(struct scsi_sense_data *sense, int *error_code,
    int *sense_key, int *asc, int *ascq);
int scsi_extract_sense_ccb(
    union ccb *ccb, int *error_code, int *sense_key, int *asc, int *ascq);
void scsi_extract_sense_len(struct scsi_sense_data *sense, u_int sense_len,
    int *error_code, int *sense_key, int *asc, int *ascq, int show_errors);
int scsi_get_sense_key(
    struct scsi_sense_data *sense, u_int sense_len, int show_errors);
int scsi_get_asc(
    struct scsi_sense_data *sense, u_int sense_len, int show_errors);
int scsi_get_ascq(
    struct scsi_sense_data *sense, u_int sense_len, int show_errors);
...
scsi_8btou64(const uint8_t *bytes)
{
	...
	rv = (((uint64_t)bytes[0]) << 56) | (((uint64_t)bytes[1]) << 48) |
