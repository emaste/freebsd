...
static const struct ng_cmdlist ng_sppp_cmds[] = {
	{ NGM_SPPP_COOKIE, NGM_SPPP_GET_IFNAME, "getifname", NULL,
...
ng_sppp_get_unit(int *unit)
{
	...
	for (index = 0;
	     index < ng_sppp_units_len && ng_sppp_units[index] == 0xFF; index++)
		;
	if (index == ng_sppp_units_len) { /* extend array */
		...
		newarray = malloc(
		    newlen * sizeof(*ng_sppp_units), M_NETGRAPH_SPPP, M_WAITOK);
		bcopy(ng_sppp_units, newarray,
		    ng_sppp_units_len * sizeof(*ng_sppp_units));
		bzero(newarray + ng_sppp_units_len, newlen - ng_sppp_units_len);
