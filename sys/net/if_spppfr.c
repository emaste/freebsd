...
sppp_fr_input(struct sppp *sp, struct mbuf *m)
{
	...
	if (h[2] != FR_UI) {
		if (debug)
			printf(SPP_FMT
			    "Invalid frame relay header flag 0x%02x\n",
			    SPP_ARGS(ifp), h[2]);
		goto bad;
	}
	switch (h[3]) {
	...
	case FR_PADDING:
		...
		if (proto == ETHERTYPE_ARP) {
			...
			if (m->m_pkthdr.len != 10 + sizeof(struct arp_req)) {
				if (debug)
					printf(SPP_FMT
					    "Bad ARP request size = %d bytes\n",
					    SPP_ARGS(ifp), m->m_pkthdr.len);
				goto bad;
			}
			sppp_fr_arp(
			    sp, (struct arp_req *)(h + 10), h[0] << 8 | h[1]);
			m_freem(m);
			return;
		}
		hlen = 10;
...
sppp_fr_arp(struct sppp *sp, struct arp_req *req, u_short his_hardware_address)
{
	...
	if ((ntohs(req->htype) != ARPHRD_FRELAY ||
		...
		if (debug)
			printf(SPP_FMT
			    "Invalid ARP hardware/protocol type = 0x%x/0x%x\n",
			    SPP_ARGS(ifp), ntohs(req->htype),
			    ntohs(req->ptype));
		return;
	}
	if (req->halen != 2 || req->palen != 4) {
		if (debug)
			printf(SPP_FMT
			    "Invalid ARP hardware/protocol address length = %d/%d\n",
			    SPP_ARGS(ifp), req->halen, req->palen);
		return;
	}
	switch (ntohs(req->op)) {
	default:
	...
	if (debug)
		printf(SPP_FMT
		    "got ARP request, source=0x%04x/%d.%d.%d.%d, target=0x%04x/%d.%d.%d.%d\n",
		    ...
		    (unsigned char)his_ip_address, my_hardware_address,
	...
	if (debug)
		printf(SPP_FMT
		    "send ARP reply, source=0x%04x/%d.%d.%d.%d, target=0x%04x/%d.%d.%d.%d\n",
		    SPP_ARGS(ifp), my_hardware_address,
		    (unsigned char)(my_ip_address >> 24),
		    (unsigned char)(my_ip_address >> 16),
		    (unsigned char)(my_ip_address >> 8),
		    (unsigned char)my_ip_address, his_hardware_address,
		    (unsigned char)(his_ip_address >> 24),
		    (unsigned char)(his_ip_address >> 16),
		    (unsigned char)(his_ip_address >> 8),
	...
	for (p = h + 6; p < h + len;) {
		...
		case FR_FLD_VERIFY:
			if (p[1] != 2) {
				if (debug)
					printf(SPP_FMT
					    "Invalid signaling verify field length %d\n",
					    SPP_ARGS(ifp), p[1]);
				break;
			}
