...
#define SPPPIOGDEFS \
	((caddr_t)(('S' << 24) + (1 << 16) + sizeof(struct sppp_parms)))
#define SPPPIOSDEFS \
	((caddr_t)(('S' << 24) + (2 << 16) + sizeof(struct sppp_parms)))
...
void sppp_get_ip_addrs(
    struct sppp *sp, u_long *src, u_long *dst, u_long *srcmask);
