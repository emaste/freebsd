    ...
    byte *UDFCompressed) /* (Output) compressed string, as bytes. */
{
	...
	if (compID != 8 && compID != 16) {
		...
		while (unicodeIndex < numberOfChars) {
			...
			UDFCompressed[byteIndex++] = unicode[unicodeIndex] &
			    0x00FF;
			unicodeIndex++;
		}
	}
 ...
 * CRC 010041
 */
static unsigned short crc_table[256] = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084,
...
unsigned short udf_cksum(s, n) unsigned char *s;
int n;
{
...
/* UNICODE Checksum */
unsigned short udf_unicode_cksum(s, n) unsigned short *s;
int n;
{
	unsigned short crc = 0;
 ...
 */
int
UDFTransName(unicode_t *newName, /* (Output)Translated name. Must be of length
				  * MAXLEN */
 ...
 */
int
UnicodeInString(unsigned char *string, /* (Input) String to search through. */
