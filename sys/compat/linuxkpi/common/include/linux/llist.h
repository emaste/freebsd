...
llist_del_first(struct llist_head *head)
{
	...
	do {
		...
		if (first == NULL)
			return NULL;
		next = first->next;
	} while (atomic_cmpset_ptr((uintptr_t *)&head->first, (uintptr_t)first,
		     (uintptr_t)next) == 0);
...
#define llist_for_each_safe(pos, n, node)                               \
	for ((pos) = (node); (pos) != NULL && ((n) = (pos)->next, pos); \
	     (pos) = (n))

#define llist_for_each_entry_safe(pos, n, node, member)                        \
	for (pos = llist_entry((node), __typeof(*pos), member); pos != NULL && \
	     (n = llist_entry(pos->member.next, __typeof(*pos), member), pos); \
	     pos = n)

#define llist_for_each_entry(pos, node, member)                     \
	for ((pos) = llist_entry((node), __typeof(*(pos)), member); \
	     (pos) != NULL; (pos) = llist_entry(                    \
				(pos)->member.next, __typeof(*(pos)), member))
