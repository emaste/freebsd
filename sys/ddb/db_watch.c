...
#ifdef notused
static bool db_find_watchpoint(vm_map_t map, db_addr_t addr, db_regs_t *regs);
#endif
static void db_list_watchpoints(void);
static void db_set_watchpoint(vm_map_t map, db_addr_t addr, vm_size_t size);
...
db_set_watchpoint(vm_map_t map, db_addr_t addr, vm_size_t size)
{
	...
	for (watch = db_watchpoint_list; watch != 0; watch = watch->link)
		if (db_map_equal(watch->map, map) && (watch->loaddr == addr) &&
...
#endif
		    db_map_current(watch->map) ? "*" : " ", (void *)watch->map,
		    (long)watch->loaddr,
...
/*ARGSUSED*/
void
db_deletewatch_cmd(db_expr_t addr, bool have_addr, db_expr_t count, char *modif)
{
...
/*ARGSUSED*/
void
db_watchpoint_cmd(db_expr_t addr, bool have_addr, db_expr_t count, char *modif)
{
	vm_size_t size;
	db_expr_t value;
...
void
db_deletehwatch_cmd(db_expr_t addr, bool have_addr, db_expr_t size, char *modif)
{
	int rc;
...
void
db_hwatchpoint_cmd(db_expr_t addr, bool have_addr, db_expr_t size, char *modif)
{
	int rc;
	...
	rc = kdb_cpu_set_watchpoint(
	    (vm_offset_t)addr, (vm_size_t)size, KDB_DBG_ACCESS_W);
	...
	case ENXIO:
		db_printf(
		    "Hardware watchpoints are not supported on this platform.\n");
		break;
	default:
		if (rc != 0)
