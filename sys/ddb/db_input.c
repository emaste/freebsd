...
static void db_putstring(s, count) char *s;
int count;
{
	while (--count >= 0)
		cnputc(*s++);
...
static void db_putnchars(c, count) int c;
int count;
{
	while (--count >= 0)
		cnputc(c);
...
#define DEL_FWD 0
#define DEL_BWD 1
static void db_delete(n, bwd) int n;
int bwd;
{
	char *p;
...
static int db_inputchar(c) int c;
{
	static int escstate;
...
int db_readline(lstart, lsize) char *lstart;
int lsize;
{
