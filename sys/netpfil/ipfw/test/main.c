...
struct cfg_s {
	...
	int32_t th_min,
	    th_max; /* thresholds for hysteresis; negative means per flow */
#ifdef USE_BURST
	int maxburst;
#endif /* USE_BURST */
	...
	int cur_y int cur_fs; /* used in generation, between 0 and max_y - 1 */
#endif			      /* USE_CUR */
	...
	/* scheduler hooks */
	int (*enq)(struct dn_sch_inst *, struct dn_queue *, struct mbuf *);
...
mainloop(struct cfg_s *c)
{
	...
	for (i = 0; i < c->loops; i++) {
		...
		controller(c);
		DX(3, "loop %d enq %d send %p rx %d", i, c->_enqueue, c->tosend,
		    c->can_dequeue);
		if ((m = c->tosend)) {
			int ret;
		...
		do_dequeue:
			...
			if (m) {
				...
				D("--- ouch, cannot operate on iteration %d, pending %d",
				    i, c->pending);
				break;
			}
		}
...
parse_flowsets(struct cfg_s *c, const char *fs)
{
	...
	if (fs != NULL) { /* first pass */
		if (c->fs_config)
			D("warning, overwriting fs %s with %s", c->fs_config,
			    fs);
		c->fs_config = fs;
	}
	...
	for (next = s; (cur = strsep(&next, ","));) {
		...
		l_steps = p ? getnum(p + 1, &p, "l_steps") :
				    (len_h == len ? 1 : 2);
		flows = getnum(strsep(&cur, ":"), NULL, "flows");
		if (flows == 0)
			flows = 1;
		DX(4, "weight %d..%d (%d) len %d..%d (%d) flows %d", w, w_h,
		    w_steps, len, len_h, l_steps, flows);
		...
		for (i = 0; i < w_steps; i++) {
			...
			for (j = 0; j < l_steps; j++, n_fs++) {
				...
				li = len +
				    ((len_h - len) * j) /
					(l_steps == 1 ? 1 : (l_steps - 1));
				x = (wi * 2048) / li;
				DX(3,
				    "----- fs %4d weight %4d lmax %4d X %4d flows %d",
				    n_fs, wi, li, x, flows);
				if (pass == 0)
					continue;
				...
				fs->index = n_fs;
				fs->n_flows = flows;
				fs->cur = fs->first_flow = prev == NULL ?
					  0 :
					  prev->next_flow;
	...
	DX(1, "%d flows on %d flowsets max_y %d", c->flows, c->flowsets,
	    c->max_y);
#endif /* USE_CUR */
	for (i = 0; i < c->flowsets; i++) {
		struct dn_fs *fs = &c->fs[i].fs;
		DX(1, "fs %3d w %5d l %4d flow %5d .. %5d y %6d .. %6d", i,
		    fs->par[0], fs->par[1], fs->first_flow, fs->next_flow,
		    fs->base_y, fs->next_y);
...
init(struct cfg_s *c)
{
	...
	D("q_len %d dn_fsk %d si %d sched %d", c->q_len,
	    (int)sizeof(struct dn_fsk), c->si_len, c->schk_len);
...
controller(struct cfg_s *c)
{
	...
	if (1) {
		...
		h = &c->ll[i];
		ND(1, "backlog %d p %p prev %p next %p", i, h, h->prev,
		    h->next);
		...
		ND(1, "before %d p %p prev %p next %p", i + 1, h + 1, h[1].prev,
		    h[1].next);
		list_add_tail(&q->ni.h, h + 1);
		ND(1, " after %d p %p prev %p next %p", i + 1, h + 1, h[1].prev,
		    h[1].next);
		if (i < BACKLOG) {
	...
	ND(2, "y %6d flow %5d fs %3d weight %4d len %4d", c->cur_y, m->flow_id,
	    c->cur_fs, fs->par[0], m->m_pkthdr.len);
}
